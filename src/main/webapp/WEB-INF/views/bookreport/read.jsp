...
<section class="bookreport-detail">
  <h2><i class="fa fa-comment-dots"></i> 문의 상세</h2>

  <div class="bookreport-box">
  <div class="bookreport-meta">
    <div><strong>번호:</strong> ${vo.bookreport_id}</div>
  ...
  <div class="bookreport-title-content">
    <h3 class="bookreport-title">제목: ${vo.title}</h3>
    <p class="bookreport-content">${vo.content}</p>
  </div>

  <div class="bookreport-back">
    <a href="/bookreport/list" class="btn-back"><i class="fa fa-arrow-left"></i> 목록으로</a>
  </div>
</div>

  
	  <c:if test="${not empty responseVO}">
	  <div class="bookreport-response">
	...
	<c:if test="${vo.status eq '접수'}">
	  ...
	  <div id="editForm" style="display:none; margin-top:20px;">
	    <form action="${pageContext.request.contextPath}/bookreport/update" method="post">
	      <input type="hidden" name="bookreport_id" value="${vo.bookreport_id}" />
	...
	<c:if test="${vo.status eq '접수'}">
    <a href="${pageContext.request.contextPath}/bookreport/delete?bookreport_id=${vo.bookreport_id}" 
